...
中央Burst SpringへのRig登録とUTJ／Legacy別の回転復元は動作確認済みです。Marie／TokoのUTJ Spring BoneとSD UnityChanのLegacy Spring Boneでは回転契約が異なるため、同じ復元式を使用しません。Legacy Spring Boneは元実装と同様に、各Solveで初期Local Rotationを基準姿勢として復元方向とDynamic Ratioの補間元を計算します。前回のばね回転を次の基準姿勢として再利用しません。
