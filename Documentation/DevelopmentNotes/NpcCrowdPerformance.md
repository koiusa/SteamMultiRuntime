...
Editor／Development Buildでは、`Tools > SteamMultiRuntime > Diagnostics > Performance > FPS Logging`を明示的にONにした場合だけ`RuntimeFrameRateLogger`が全描画フレームをサンプリングし、1秒ごとに`[FrameRate]`を出力します。設定は永続化され、Play中の切替も即時反映します。初期値はOFFです。
