...
まだnpmへ存在しないパッケージにはTrusted Publisherを設定できません。そのパッケージだけはローカルで`npm login`して初回公開し、作成後に同じ`release.yml`をTrusted Publisherとして登録します。
...
公開対象の現在バージョンは、`com.koiusa.input.core`が`0.2.0`、`com.koiusa.application`が`0.2.0`、`com.koiusa.ui.core`が`0.1.0`、`com.koiusa.input.icons`が`0.1.0`、`com.koiusa.keyconfig`が`0.2.0`、`com.koiusa.inputguide`が`0.1.0`です。Keyconfigは`input.core` 0.2.0、`ui.core` 0.1.0、`input.icons` 0.1.0を推移依存として導入します。
...
- SubmitおよびPreviousSection／NextSectionの解放待ちと入力診断表示は、毎フレームの`Update`ではなくInput Actionの解放通知とState／DeltaStateイベントで更新します。
...
- 1行表示した1／2修飾Compositeでは、Modifier 1、Modifier 2、本体Buttonの各アイコンを組合せ順に表示します。
