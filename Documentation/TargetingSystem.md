...
標準Prefabは必要なフレーミングComponentを事前配置し、欠落時はRuntime生成せず警告を出します。Player生成後に必要になるCamera Follow Targetだけは、Camera Rigへ事前配置した`TargetingCameraRuntimeObjectFactory`が生成と破棄を一元管理します。Follow TargetはPlayer配下に生成されるため、Player Despawnで先に破棄された場合もPresenterはUnityの破棄済み参照を破棄し、次のLocal Player登録時に再生成します。
...
Multi Lock中はStrafe入力の有無にかかわらず、移動入力方向ではなくPrimary Targetを向きます。`TargetingFacingRequestSource`は状態変更CallbackでPrimaryのAimPointを保持し、Locomoterの共通`ActorFacingRequestResolver`へTargeting PriorityのRequestを提供します。Network PlayerではこのRequestを入力状態としてServerへ送り、Server側Motorが回転を確定します。Wire Groundなど高Priorityの移動制約がある場合はそちらを優先します。
