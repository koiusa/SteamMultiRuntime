...
- Input GuideはF1入力を`InputActionBinding`で所有し、疑似デバイス表示とOperationパネルを同じInputActionAssetから構築します。`InputGuideSelectionController`はInspectorで参照したOverlayへAction Map群とBinding Groupを一貫したスナップショットとしてpushします。候補列挙にもOverlayの`Input Actions Config`を使い、Controller側ではInput Actionsを二重管理しません。Map複数選択は専用EditorのマスクUIからMap名リストへ保存します。コンパクト表示のMapタブ切替と一覧スクロールは独立した`InputGuideNavigationController`が所有し、参照先OverlayのInput Actions ConfigからActionを選択・購読します。`InputGuideOverlay`は入力監視と表示モードを、`InputGuideOperationPanel`は操作一覧の生成とデバイス別表示を所有します。画面側の表示境界は`IInputGuideOverlay`で、`InputGuideConfiguration`を取得・一括適用して一時表示から通常表示へ復元できます。既定の`Standard`プリセットではOperationパネルは従来どおり画面上部の全幅を使います。`CompactOperations`は右上440pxの公式プリセットで、複数Mapをタブ切替し、利用側の追加USSや内部Element参照を必要としません。Input Guideは`UIDocument.sortingOrder`を所有せず、合成側が表示順を決定します。
