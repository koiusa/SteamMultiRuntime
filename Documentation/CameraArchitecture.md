...
Awake時に配下の全`CinemachineCamera`を走査し、`CinemachineDeoccluder`と`CinemachineDecollider`がなければ追加して共通設定を適用します。Prefab側にComponentがある場合もInspector値で再設定されます。
...
2. Mixing Camera配下の全Virtual Cameraへ同じ設定を適用できるか
3. Grapple中にCamera入力が停止し、終了後に復帰するか
4. 遮蔽物の出入りでCameraが振動しないか
5. CameraがCollider内部へ入った場合に速やかに押し出されるか
6. LocalとLobbyの両ContextでWeight切替が動作するか
